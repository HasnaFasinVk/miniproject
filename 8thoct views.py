...
def approvalform_post(request):
    ...
    Request.objects.filter(id=id).update(status="selected")
...
def managecastadd(request):
    mca=Managecastingmain.objects.all()
    return render(request,'Admin/manage castiing-casting call add.html',{'data':mca})
...
def managecastadd_post(request):
    ...
    objms.disablecastingstatus="pending"
    objms.save()
...
def disablecasting(request,id):
    Managecastingsub.objects.filter(MANAGECASTINGMAIN_id=id).update(disablecastingstatus='Disable')
    return HttpResponse("<script>alert('Casting Call Disabled');window.location='/myapp/managecastingview/'</script>")
...
def managecastingview(request):
    objmv=Managecastingsub.objects.filter(disablecastingstatus='pending')
    return render(request,'Admin/manage casting-view.html',{'datamv':objmv})
...
def selectedcandidates(request,id):
    sc=Request.objects.filter(CASTING_id=id)
    return render(request,'Admin/Selected candidates VIEW.html',{'data1':sc})
...
def histsearch(request):
    casting_film=request.POST['textfields']
    objmv=Managecastingsub.objects.filter(MANAGECASTINGMAIN__casting_film__icontains=casting_film)
    return render(request, 'Admin/history casting view ad.html',{'datamv':objmv})


def historycastingviewad(request):
    hcv=Managecastingsub.objects.filter(disablecastingstatus='disable')
    return render(request,'Admin/history casting view ad.html',{'datamv':hcv})

def historycastingviewad_post(request):
    hvca=Managecastingsub.objects.all()
    return render(request, 'Admin/history casting view ad.html',{'datamv':hvca})
...
def hsearch(request):
    ...
    return render(request, 'user/history casting view us.html',{'data9':objcs})
def userviewhistory(request):
    objcs=Managecastingsub.objects.filter(disablecastingstatus='disable')
    return render(request,'user/history casting view us.html',{'data9':objcs})
def userviewhistory_post(request):
    obj9=Managecastingsub.objects.all()
    return render(request,'user/history casting view us.html',{'data9':obj9})
...
def approvalformuse_post(reqest):
    obj8=Selectedcandidates.objects.filter(REQUEST__USER__LOGIN_id=reqest.session['lid'])
    return render(reqest, 'user/Approval formuse.html',{'data8':obj8})
...
def castinginfoview(request):
    objcs=Managecastingsub.objects.filter(disablecastingstatus='pending')
    return render(request,'user/CASTING INFO VIEW-22.html',{'data9':objcs})
