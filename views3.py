...
def achievementsadd_post(request):
    ...
    return HttpResponse("<script>alert('Achievements Added');window.location='/myapp/userhome/'</script>")
...
def achievementsedit_post(request,id):
    ...
    Achievement.objects.filter(id=id).update(date=date,achievements=achievements,experience=experience,instagram_id=instagram_id)
